...
// Example PHP script for handling status updates
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $data = json_decode(file_get_contents('php://input'), true);
    $status = $data['status'];
    $agent_id = $_SESSION['agent_id']; // Get the agent's ID from the session

    // Update the agent's status in the VICIdial database
    $query = "UPDATE vicidial_users SET status = ? WHERE user_id = ?";
...
// PHP script to handle answering calls
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $data = json_decode(file_get_contents('php://input'), true);
    $callId = $data['callId'];

    // Handle call answering logic
    // Update VICIdial with the new call status
    // ...
